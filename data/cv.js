// CV/Resume data configuration
window.cvData = {
    education: [
        {
            institution: "Monash University Malaysia",
            degree: "Bachelor of Computer Science in Data Science, School of Information Technology",
            period: "2024.07 - Present",
            description: "GPA 4.0、主席表彰、奨学金獲得",
            url: "https://www.monash.edu.my"
        },
        {
            institution: "Sunway College",
            degree: "Monash University Foundation Year (MUFY)",
            period: "2023.07 - 2024.06",
            description: "GPA 4.0（WAM93.25%）、成績優秀表彰、奨学金獲得"
        },
        {
            institution: "公立高等学校（国際バカロレア認定校）",
            degree: "普通科",
            period: "2020.04 - 2023.03",
            description: "GPA 4.0"
        }
    ],
    skills: [
        {
            category: "Frontend Development",
            skills: [
                { name: "HTML/CSS", level: 5 },
                { name: "JavaScript", level: 4 },
                { name: "React", level: 3 },
                { name: "Next.js", level: 3 },
                { name: "Astro", level: 2 }
            ]
        },
        {
            category: "Backend Development",
            skills: [
                { name: "Python", level: 5 },
                { name: "R", level: 4 },
                { name: "TypeScript", level: 3 },
                { name: "SQL", level: 2 },
                { name: "PHP", level: 1 }
            ]
        },
        {
            category: "Development Tools",
            skills: [
                { name: "Git/GitHub", level: 5 },
                { name: "WordPress", level: 5 },
                { name: "Dify", level: 4 },
                { name: "MySQL", level: 3 },
                { name: "Firebase", level: 3 },
                { name: "Linux", level: 2 },
                { name: "Figma", level: 2 },
                { name: "AWS", level: 2 }
            ]
        },
        {
            category: "Other Tools",
            skills: [
                { name: "Sheet Music Software (MuseScore, Sibelius, Dorico Pro)", level: 5 },
                { name: "Logic Pro", level: 4 },
                { name: "OBS Studio", level: 4 },
                { name: "Final Cut Pro", level: 2 }
            ]
        },
        {
            category: "Languages",
            skills: [
                { name: "Japanese: Native", level: 5 },
                { name: "English: Business", level: 4 },
                { name: "Malay / Indonesian: Basic", level: 1 }
            ]
        },
        {
            category: "Specialized Skills",
            skills: [
                {
                    name: "Data Science",
                    level: 4,
                    description: [
                        "学部課程でコンピュータサイエンスとデータサイエンスを専攻",
                        "モナッシュ大学で数学分野における主席表彰",
                        "日本人工知能オリンピック金賞・日本5位",
                        "カンガルー数学コンテスト（世界最大の数学コンテスト）銀賞"
                    ]
                },
                {
                    name: "Research / Writing",
                    level: 5,
                    description: [
                        "ライターとして1ヶ月でGoogle AdSense審査合格、5か月で月1万PVを達成、累計700記事以上執筆",
                        "文章の論理構造や読みやすさに定評あり",
                        "複数の広告案件獲得",
                        "高校時代の研究論文コンテストで入賞多数"
                    ]
                },
                {
                    name: "Project Management / Business",
                    level: 4,
                    description: [
                        "複数の業界でのインターンシップ経験",
                        "ウェブサイト運営や音楽事業を収益化",
                        "52Hz Acceleratorや家庭教師でのメンター経験",
                        "コミュニティ運営やサポート業務の担当経験あり"
                    ]
                },
                {
                    name: "Music",
                    level: 5,
                    description: [
                        "6音程度まで聴音（耳コピ）が可能",
                        "YouTubeチャンネル登録者数1万人超",
                        "オンラインストアでのピアノ楽譜販売、オーダーメイド楽譜の制作",
                        "音楽講師の経験あり"
                    ]
                }
            ]
        }
    ],
    awards: {
        "2025": [
            {
            title: "YouTube: チャンネル登録者数1万人達成",
            organization: "YouTube",
            date: "2025.06",
            description: "ピアノの演奏動画や制作した楽譜の動画を投稿",
            link: "https://www.youtube.com/@ray_pianocover"
            },
            {
            title: "日本人工知能オリンピック2025: 金賞、オープン枠 全国5位",
            organization: "国際人工知能オリンピック日本委員会",
            date: "2025.05",
            description: "Kaggle形式のデータ分析コンペ",
            link: "https://ioai-japan.org/"
            },
            {
            title: "モナッシュ大学 情報学部: 成績優秀賞受賞",
            organization: "Monash University Malaysia",
            date: "2025.04",
            description: "全科目総合での成績優秀賞を受賞。WAM約90%"
            },
            {
            title: "モナッシュ大学 情報学部: 主席表彰受賞",
            organization: "Monash University Malaysia",
            date: "2025.04",
            description: "数学分野における主席表彰を受賞"
            },
            {
            title: "東京大学AI経営講座 AI Business Insights 2025: 修了",
            organization: "東京大学 松尾研究室",
            date: "2025.04",
            description: "ビジネスにおけるAIの活用方法についての知見を深めた",
            link: "https://weblab.t.u-tokyo.ac.jp/lecture/course-list/ai-driven/"
            },
            {
            title: "20歳のつどい 同級生応援イベントにて、経歴掲載",
            organization: "地元市役所",
            date: "2025.01",
            description: "イベント後、市役所にて一定期間の経歴掲載"
            },
            {
            title: "2025年日本言語学オリンピック: 敢闘賞",
            organization: "国際言語学オリンピック日本委員会",
            date: "2025.02",
            description: "未知の言語を解析する学術オリンピック",
            link: "https://iolingjapan.org/"
            }
        ],
        "2024": [
            {
            title: "グローバル消費インテリジェンス寄附講座 2024 Summer: 修了",
            organization: "東京大学 松尾研究室",
            date: "2024.08",
            description: "データサイエンスの基礎やPythonでの機械学習の実装について学んだ",
            link: "https://gci2.t.u-tokyo.ac.jp/"
            },
            {
            title: "カンガルー数学コンテスト 2024: 銀賞",
            organization: "Kangaroo Math Malaysia",
            date: "2024.09",
            description: "世界最大の数学コンテスト",
            link: "https://www.kangaroomath.com.my/"
            },
            {
            title: "言語学オリンピックマレーシア大会2024: 銅賞、ベスト32",
            organization: "Malaysian Computational Thinking Association",
            date: "2024.03",
            description: "マレーシア国内でベスト32の成績を収め、決勝トーナメントにも進出",
            link: "https://myclo.my/"
            },
            {
            title: "日本言語学オリンピック: 第1問・第2問敢闘賞",
            organization: "国際言語学オリンピック日本委員会",
            date: "2024.02",
            description: "未知の言語を解析する学術オリンピック",
            link: "https://iolingjapan.org/"
            }
        ],
        "~2023": [
            {
            title: "研究科学コンテスト: 全国最優秀賞、全国努力賞、国際大会出場 ほか",
            organization: "NA",
            date: "~2023",
            description: "合成音声（Text-to-Speech）の品質改善に関する、音響音声学領域の研究を行った"
            },
            {
            title: "名大MIRAI GSC: 第2ステージ修了",
            organization: "名古屋大学",
            date: "~2023",
            description: "大学の研究室と共同で超伝導に関する研究を行った",
            link: "https://nuqa.nagoya-u.ac.jp/miraigsc/"
            },
            {
            title: "毎パソ冬季大会: 和文B 初段認定・高校生部門全国3位 ほか",
            organization: "毎日新聞社 ほか",
            date: "~2023",
            description: "タイピング技能で初段認定を受け、高校生部門で全国3位の成績を収めた",
            link: "https://pasoken.or.jp/maipaso/"
            },
            {
            title: "将棋: 県大会 優勝（団体）・第3位（個人）、東日本大会出場 ほか",
            organization: "県高等学校文化連盟 ほか",
            date: "~2023",
            description: "中学校から高校にかけて、将棋の大会に出場した"
            },
            {
            title: "英語ディベート: 県大会 第3位（団体）・ベストディベーター賞（個人）",
            organization: "県高等学校教育研究会",
            date: "~2023",
            description: "4人チームで参加し、自身はアタッカーの役割を務めた"
            }
        ]
    },
    experience: [
        {
            company: "株式会社リアブロード（スマ留）",
            period: "2025.05 - Present",
            position: "業務委託（公式アンバサダー）",
            description: "留学関係のイベント登壇・運営、記事執筆など",
            url: "https://smaryu.com/"
        },
        {
            company: "株式会社GFIエデュケーションズ（Go for it）",
            period: "2024 - Present",
            position: "インターンシップ",
            description: "留学関係の記事執筆、WordPressウェブサイトの構築、SEO対策など",
            url: "https://go-for-it-malaysia.com/"
        },
        {
            company: "Tobiratory Japan",
            period: "2024 - Present",
            position: "インターンシップ",
            description: "AIを用いた業務自動化ツールの作成、JavaScriptフレームワークを用いたウェブサイト制作など",
            url: "https://www.tobiratory.com/"
        },
        {
            company: "[REDACTED]",
            period: "2024",
            position: "インターンシップ（[REDACTED]）",
            description: "留学関係のイベント登壇・運営、通訳など",
            url: "[REDACTED]"
        },
        {
            company: "Self-Employed",
            period: "2024",
            position: "家庭教師",
            description: "中学数学の個別指導など"
        },
        {
            company: "株式会社ココナラ",
            period: "2023 - Present",
            position: "フリーランス音楽講師・楽譜制作",
            description: "フリーランス音楽講師およびオーダーメイド楽譜制作など",
            url: "https://coconala.com/users/4454434"
        },
        {
            company: "株式会社LOOPAL",
            period: "2023 - Present",
            position: "インターンシップ",
            description: "留学関係の記事執筆、SEO対策など",
            url: "https://loopal.net/"
        },
        {
            company: "一般社団法人52Hz",
            period: "2023 - Present",
            position: "会員、業務委託（52Hz Accelerator 0期メンター）",
            description: "中高生の進路サポートや課外活動のメンターなど",
            url: "https://52hz.world/"
        }
    ],
    certifications: [
        {
            title: "基本情報技術者試験",
            organization: "独立行政法人情報処理推進機構（IPA）",
            date: "2025.02"
        },
        {
            title: "ITパスポート試験",
            organization: "独立行政法人情報処理推進機構（IPA）",
            date: "2025.02"
        },
        {
            title: "日本商工会議所 簿記検定試験 3級",
            organization: "日本商工会議所",
            date: "2025.02"
        },
        {
            title: "Mahout (象使い) 免許資格",
            organization: "Manifa Elephant Camp（ラオス）",
            date: "2024.06"
        },
        {
            title: "普通自動車第一種運転免許（AT限定）",
            organization: "日本国公安委員会",
            date: "2023.05"
        }
    ],
    grants: [
        {
            title: "Monash High Achiever Award",
            organization: "Monash University Malaysia",
            amount: "MYR 10,000",
            date: "2025",
            description: "成績優秀者として授与された奨学金。1セメスター（半年）ごとにMYR5000が支給"
        },
        {
            title: "Monash High Achiever Award", 
            organization: "Monash University Malaysia",
            amount: "MYR 10,000",
            date: "2024",
            description: "成績優秀者として授与された奨学金。1セメスター（半年）ごとにMYR5000が支給"
        },
        {
            title: "Pre-U Bursary for Overseas Qualification",
            organization: "Sunway College",
            amount: "MYR 5,000",
            date: "2023",
            description: "成績優秀者として授与された奨学金。1セメスター（半年）ごとにMYR5000が支給"
        }
    ]
};
